...
exports.generate = onCall(
  ...
  async (req) => {
    ...
    const config = configSnap.exists ? configSnap.data() : { templates: [], productContext: "", glossary: "" };
...
function buildPrompt(mode, { config, spec, sectionKey }) {
  const templateDesc = (spec.sections || [])
    .map((s) => `- ${s.title} (${s.key}): ${s.hint || "(지시사항 없음)"}`)
    .join("\n");
  ...
  if (mode === "draft") {
    return `${context}\n\n다음 ${spec.docType || "문서"} 섹션 템플릿에 맞춰, 한 줄 문제 정의 "${spec.oneLiner}"를 바탕으로 각 섹션의 초안을 작성하라. 각 섹션의 지시문(hint)을 반드시 따를 것.\n\n${templateDesc}\n\n각 섹션을 "## 섹션키" 형식의 마크다운 헤더로 구분해서 출력하라.`;
  }
  if (mode === "section") {
    const target = (spec.sections || []).find((s) => s.key === sectionKey);
    const hintText = target?.hint || "(지시사항 없음 — 섹션 제목만 보고 판단할 것)";
    return `아래는 작성 중인 ${spec.docType || "문서"} 전체다(참고용 컨텍스트):\n\n${docText}\n\n${context}\n\n이제 "${target?.title}" 섹션만 다시 작성하라. 지시문: ${hintText}\n요청한 섹션의 본문만 출력하고 다른 섹션은 출력하지 마라.`;
  }
  // critique
  return `아래는 검토할 ${spec.docType || "문서"}다:\n\n${docText}\n\n${context}\n\n문서를 고치지 말고, 리뷰 전에 점검해야 할 질문을 정확히 3개만 제시하라. 각 질문은 한 줄로.`;
}
