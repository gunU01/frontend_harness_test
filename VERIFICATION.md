...
## P1-h — 비주얼 기본기 개선 (이번에 추가)

사용자가 실제 화면을 보고 "와이어프레임 같다"고 지적한 뒤 진행한 시각적 기본기 패스입니다. 레이아웃/로직은 그대로 두고 Tailwind 클래스만 조정했습니다(표면 분리용 `bg-slate-50`, 카드 `shadow-sm`/`hover:shadow-md`, 사이드바 nav의 hover/active 배경 pill, 인풋 `focus:ring`, 빈 상태 점선 패널).

- [x] **실제 화면 육안 확인** — 자동화 아님, 수동으로 직접 확인함: 에뮬레이터(`firebase emulators:start --only auth,firestore`)를 띄우고 `VITE_USE_FIREBASE_EMULATOR=true`로 dev 서버를 연결한 뒤, `SignIn.tsx`의 e2e 전용 로그인 경로(`?e2eEmail=&e2eName=`)로 로그인해서 Home(빈 상태/프로젝트 있는 상태)·ProjectDetail·SpecEditor·SpecList·Community·Analytics 6개 화면을 Playwright로 스크린샷 비교(적용 전/후). 카드에 그림자가 생기고 사이드바 hover에 배경이 생기는 것, Analytics의 각 차트가 카드로 감싸진 것을 직접 확인함.
- [ ] **자동 시각 회귀 테스트는 없음** — 이번 변경은 순수 className 조정이라 기존 E2E 스위트(`npm run test:e2e`)의 텍스트/역할 기반 단언은 전부 그대로 통과하지만, "그림자가 실제로 렌더링되는지" 같은 픽셀 단위 검증은 스위트에 없음 — 스크린샷 비교는 이번에 사람이 직접 했고, 다음에 또 손대면 다시 수동으로 봐야 함.
- [ ] **실제 레퍼런스 반영 전 단계**: 이번 패스는 "기본기"만 고친 것이고(그림자/여백/호버), 사용자가 원하는 구체적인 비주얼 방향(참고 스크린샷 기반)은 아직 반영 전 — 다음 확장 대상.
