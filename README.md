...
한 줄 문제 정의만 넣으면 내 문서 템플릿(섹션 + hint)에 맞춰 AI가 초안을 채워주는 개인용 도구입니다. 처음엔 PRD 한 종류만 지원했지만, "PM을 위한 문서 작성 도구"로 포지셔닝을 넓히면서 회의록 등 여러 문서 타입을 등록·전환하고, 완성한 문서를 팀에 공개 공유할 수 있게 확장했습니다. 개인이 편하게 쓰려고 만든 도구이고, 기획→개발→배포를 하네스(오케스트레이터 + 서브에이전트: planner/builder/styler/reviewer)로 진행했습니다.
...
- **v6 (OKLCH 컬러 시스템 재구축)**: 사용자가 토스 디자인 플랫폼팀의 컬러 시스템 개편 아티클을 공유하며 그 원칙을 반영해달라고 요청했습니다(`PLANNING.md` 7단계 참고). 반영한 세 원칙과 의도적으로 안 만든 것:
  - **지각 균일 명도(OKLCH)**: 기존 `primary` 램프는 몇 달 전 사용자가 준 레퍼런스를 그대로 베낀 값(v3)이라, 색상마다 "같은 숫자 단계"의 실제 명도/대비가 제각각이었습니다(아티클의 예시: Grey 100은 흰 배경 대비 1.1:1, Red 100은 1.34:1). 스크래치패드에 일회성 Node 스크립트(`culori`를 `npm install --no-save`로 임시 설치, 레포에는 안 들어감)를 작성해 50~900 각 단계마다 **모든 색상이 공유하는 OKLCH 목표 명도(L)**를 정하고, sRGB 감마트를 벗어나지 않도록 단계별로 채도(chroma)를 클램프한 뒤 hex로 변환했습니다. 생성한 hex를 다시 OKLCH로 역변환해서 목표 L과 0.006 이내로 맞는지 전부 검증했습니다(허수 없이). 결과: 새 `slate-100`(≈1.17:1)과 `red-100`(≈1.19:1)이 이제 서로 비슷한 대비를 가짐 — 예전처럼 색마다 들쭉날쭉하지 않습니다.
  - **다크 옐로우 문제**: 경고색으로 쓰는 `amber`에 공유 명도 곡선을 그대로 적용하면 어두운 단계(700~900)가 누런 갈색/올리브로 보이고, 밝은 단계는 채도를 억지로 끌어올려야 "노랑"으로 보여서 작은 텍스트로 쓰기엔 부적합했습니다. `amber`만 별도 명도 곡선(어두운 단계를 다른 색보다 의도적으로 더 밝게 유지)과 더 낮은 채도 상한을 따로 튜닝해서, 모든 단계에서 "황색"으로 읽히고 텍스트로도 쓸 수 있게 했습니다 — v3에서 배지 텍스트 대비 때문에 `status/*` 500 대신 700 계열을 쓴 것과 같은 종류의 "숫자를 맹목적으로 강제하지 않는" 예외입니다. (덤으로 새 `amber-700` 대비가 2.38:1 → 6.82:1로 올라가서 v3가 우회했던 배지 텍스트 대비 문제 자체가 사실상 해소됐습니다.)
  - **시맨틱 토큰 네이밍(Target × Role × Variant)**: `src/pages`/`src/components` 전체를 grep해서 실제 쓰이는 색 조합(`bg-primary-500`, `text-slate-600`, `border-slate-300`, `bg-green-100 text-green-700` 같은 상태 배지 등)을 먼저 인벤토리한 뒤, Figma Foundations의 시맨틱 변수 컬렉션(`text/primary`, `background/brand` 같은 기존 이름)을 **Target(fill/text/border) × Role(brand/neutral/status-success/status-warning/status-error) × Variant(default/weak/alt 등)** 이름으로 리네임했습니다(변수 ID는 그대로 유지 — 이미 바인딩된 곳은 안 깨짐). `CONVENTIONS.md`의 "스타일 (Tailwind)" 절에 시맨틱 이름 → 실제 Tailwind 클래스 매핑표를 추가했습니다.
  - **엔터프라이즈 인프라는 의도적으로 안 만듦**: 아티클의 실제 배경은 Token Studio + GitHub PR 워크플로 + Style Dictionary + 5개 플랫폼 코드젠 파이프라인이지만, 이건 여러 팀·여러 플랫폼을 위한 회사 규모 인프라입니다. 이 프로젝트는 Figma 파일 하나 + `tailwind.config.js` 하나가 전부인 1인 도구라 적용 규모가 다르므로(`PLANNING.md` 7단계에 이미 이 판단이 기록되어 있음), 원칙만 가져오고 파이프라인은 만들지 않았습니다.
...
스코프를 넓히지 않고 지금 필요한 것만 만들기 위해, 뺀 기능과 그 이유를 의도적으로 남겨둡니다.
...
각 항목은 "언젠가 하면 좋은 기능"이 아니라 "지금 스코프에서 명시적으로 뺀 이유가 있는 결정"입니다 — 나중에 왜 안 만들었는지 스스로 잊지 않기 위한 의사결정 로그이기도 합니다.
...
지금까지는 "문서를 쓰고 공유한다"까지였고, "무엇을 결정하는가" 자체를 도구 기능으로 직접 지원하는 건 없었습니다. 이번엔 네 개의 서브에이전트를 데이터 레이어 → UI 두 단계로 나눠 병렬로 진행했습니다(각자 건드리는 파일을 분리해 충돌 없이).
